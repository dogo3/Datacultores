...
# dfMAPAConsumo_Gr<-dfMAPAConsumo %>% group_by(Producto,CCAA,Fecha) %>% summarise(`Diff Volumen (miles de kg)`=`Volumen (miles de kg)`,`Volumen (miles de kg)`=`Volumen (miles de kg)`)


#Simplifico el df
dfMAPAConsumo_S<-dfMAPAConsumo %>%select(Producto,CCAA,Fecha,`Volumen (miles de kg)`) %>% filter(Producto %in% c("AGUACATE","AJOS"), CCAA=="Total Nacional")


# dfMAPAConsumo_Gr<-dfMAPAConsumo_S %>% group_by(Producto,CCAA,Fecha) %>% summarise(`Lagged Volumen (miles de kg)`=dplyr::lag(`Volumen (miles de kg)`),`Volumen (miles de kg)`=`Volumen (miles de kg)`)

dfMAPAConsumo_Gr<-dfMAPAConsumo_S %>%arrange(Producto,CCAA,Fecha)%>% group_by(Producto,CCAA)%>%mutate(laggedVolume =dplyr::lag(`Volumen (miles de kg)`))
  


# dfMAPAConsumo_Gr$laggedVolume <-dplyr::lag(dfMAPAConsumo_Gr$`Volumen (miles de kg)`) 

# summarise(`Volumen (miles de kg)`=`Volumen (miles de kg)`) %>% group_by(Producto,CCAA)

dfMAPAConsumo_Gr$`Diff Volumen (miles de kg)`<-dfMAPAConsumo_Gr$`Volumen (miles de kg)`-dfMAPAConsumo_Gr$laggedVolume

# dfMAPAConsumo_Gr %>% summarise(`Diff Volumen (miles de kg)`=`Diff Volumen (miles de kg)`)


View(dfMAPAConsumo_Gr)

p <- dfMAPAConsumo_Gr%>%
  ggplot()+
  geom_line(aes(x=Fecha, y=`Diff Volumen (miles de kg)`,col=Producto), size=1)+
  ...
  ggtitle('Precio medio - 5 productos con más volumen')+ theme(legend.position = "none")
...
View(dfComercioExterior%>%mutate(REPORTER_NEW=traslateCountry(country = REPORTER,from = "NombresComercioExterior",to="Comun")))
unique(dfComercioExterior$REPORTER)
View(dfComercioExterior)

country <- c("Belgium (incl. Luxembourg 'LU' -> 1998)","Belgium (incl. Luxembourg 'LU' -> 1998)")
from <- "NombresComercioExterior"
to <- "Comun"
l<-list(c())
for(i in seq_len(length(country))){
    l[[i]] <-   conversionPaises[conversionPaises[[from]]==country[i],to][1]
...
translateCountry(country,from="NombresComercioExterior",to="Comun")
...
```{r}
#TODO comprobar que la TC está bien calculada
