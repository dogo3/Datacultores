...
#Simplifico el df
# dfMAPAConsumo_S<-dfMAPAConsumo %>%dplyr::select(Producto,CCAA,Fecha,`Volumen (miles de kg)`) %>% filter(Producto %in% c("AGUACATE","AJOS"), CCAA=="Total Nacional")
# 
# 
...
# saveRDS(df,file="data/app/MAPAConsumo.rds")
...
# saveRDS(dfComExtTreemap,"./data/app/ComExtTreemap.rds")
```
...
# View(dfCovid)
# df<-dfCovid%>%dplyr::select(geoId,countriesAndTerritories,continentExp,dateRep,day,month,`Cumulative_number_for_14_days_of_COVID-19_cases_per_100000`,pop=`popData2019`)%>%rename(IA14=`Cumulative_number_for_14_days_of_COVID-19_cases_per_100000`)
# saveRDS(df,"data/app/COVID.rds")
```
